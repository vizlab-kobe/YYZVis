...
void CellByCellMetropolisSampling::generate_particles( const kvs::StructuredVolumeObject* volume )
{
    ...
    local::CellByCellSampling::ParticleDensityMap density_map;
    ...
    local::CellByCellSampling::GridSampler<T> sampler( &interpolator, &density_map );
        ...
        for ( kvs::UInt32 y = 0; y < ncells.y(); ++y )
        {
            for ( kvs::UInt32 x = 0; x < ncells.x(); ++x )
            {
                ...
                while ( counter < nparticles )
                {
                    ...
                    {
                        if ( ratio >= local::CellByCellSampling::RandomNumber() )
                        {
                            sampler.acceptTrial( color_map );
                            density = density_trial;
                            counter++;
...
{
    kvs::CellBase* cell = local::CellByCellSampling::Cell( volume );
    if ( !cell )
    {
        BaseClass::setSuccess( false );
    ...
    // Generate particles for each cell.
    local::CellByCellSampling::CellSampler sampler( cell, &density_map );
